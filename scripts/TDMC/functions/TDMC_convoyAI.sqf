...
for "_i" from 0 to ((count _creationarray) - 1) do
{
	...
	if (count _group_array > 0) then 
	{
		...
		_veh setDir ([_start, (_positions select 1)]call BIS_fnc_dirTo);
		_veh setPosATL _start;
		_veh setVelocity [0,0,-1];
		_veh call TDMC_fnc_garbageEH;
		_convoy set [count _convoy, _veh];
		TDMC_mission_units set [count TDMC_mission_units, _veh];
		...
		{
			if ((_veh emptyPositions "cargo") > 4) then
			{
				...
				for "_p" from 1 to (_veh emptyPositions "cargo") do
				{
					if (_p > 7) exitWith {};
					...
					_guy = _grp2 createUnit [_unit, getPosATL _veh, [], 0, "CAN_COLLIDE"];
					_guy call TDMC_fnc_garbageEH;
					_guy call TDMC_fnc_loadouts;
					_guy assignAsCargo _veh;
					_guy moveInCargo _veh;
				};
			};
		};
	};
